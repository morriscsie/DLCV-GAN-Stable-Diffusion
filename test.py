...
def test_wo_domain(model, data_loader, device):
    """Evaluate model for dataset."""
    # set eval state for Dropout and BN layers
    model.eval()
...
def generate(model, data_loader, device, flag):
    """Evaluate model for dataset."""
    # set eval state for Dropout and BN layers
    model.eval()
    # evaluate network
    path_label = []
    for (image, label, img_name) in data_loader:
        image = image.to(device)
        label = label.to(device)  #labels = labels.squeeze(1)
        img_name = img_name[0]
        size = len(label)
        preds, domain = model(image, alpha=0)
        pred_cls = preds.data.max(1)[1]
        path_label.append((img_name, pred_cls.item()))
    path_label.sort(key=lambda tup: tup[0].split(".")[0])  # sorts in place
