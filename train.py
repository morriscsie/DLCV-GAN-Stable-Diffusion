...
from test import test,test_wo_domain
from utils import save_model
import torch.backends.cudnn as cudnn
cudnn.benchmark = True

def train_src(model, params, src_data_loader, tgt_data_loader_eval, device):
    """Train dann."""
    ####################
    # 1. setup network #
    ####################
    ...
    for epoch in range(params.num_epochs):
        ...
        #print("len={}".format(len(src_data_loader)))
        for step, (images_src, class_src) in enumerate(src_data_loader):
            ...
            src_class_output = model(input_data=images_src)
        ...
        #src_test_loss, src_acc = test_wo_domain(model, src_data_loader_eval, device)
        #print("src_acc = {:.2f}% , src_loss = {:.2f}, global_step = {:d}".format( src_acc*100, src_test_loss, global_step))
        tgt_test_loss, tgt_acc = test_wo_domain(model, tgt_data_loader_eval, device)
        print("tgt_acc = {:.2f}% , tgt_loss = {:.2f}, global_step = {:d}".format( tgt_acc*100, tgt_test_loss, global_step))
        if( tgt_acc >= best_tgt_acc ):
            best_tgt_acc = tgt_acc
    print("best_acc = {:.2f}%".format(best_tgt_acc*100))
    return model

def train_dann(model, params, src_data_loader, tgt_data_loader,  src_data_loader_eval, tgt_data_loader_eval, device, PATH):
    """Train dann."""
    ####################
    # 1. setup network #
    ####################
    ...
    for epoch in range(params.num_epochs):
        ...
        #print("src_test_loss = {:.2f} , global_step = {:d}".format(src_test_loss , global_step))
        print("src_acc = {:.2f}% , global_step = {:d}".format( src_acc*100, global_step))
        print("src_acc_domain = {:.2f}% , global_step = {:d}".format(src_acc_domain*100 , global_step))
        #print("tgt_test_loss = {:.2f} , global_step = {:d}".format(tgt_test_loss , global_step))
        print("tgt_acc = {:.2f}% , global_step = {:d}".format( tgt_acc*100, global_step))
        print("tgt_acc_domain = {:.2f}% , global_step = {:d}".format(tgt_acc_domain*100 , global_step))
        if( tgt_acc >= best_tgt_acc ):
            torch.save({
                'epoch':(epoch+1),
